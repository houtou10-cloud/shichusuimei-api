...
- evidence は「計算済み事実 → 意味・解釈」の流れが顧客に分かる文章にしてください。
...
- 相談内容に関連するセクションでは、相談者の問いを明示的に直接取り上げたうえで、計算済み事実 → 解釈 → 複数の選択肢 → 現実的な行動の順で説明してください。
- 相談内容に直接関係しないセクションへ、無理に相談テーマを持ち込まないでください。
- 健康セクションでは、五行・日主・身強身弱・用神・大運・歳運から、顧客入力にない具体的な身体状態・症状・体質・生活習慣を推測しないでください。
- 健康セクションでは、睡眠・睡眠の質・疲労・呼吸・姿勢・運動・食事・臓器などを占術上の偏りから直接導かないでください。
- health の summary / detail / evidence / advice では、入力に存在しない健康事実を作らず、「生活全体のバランスを見直す参考」など一般的な表現を優先してください。
- 顧客が健康上の悩みを入力している場合でも、その内容を四柱推命から医学的に説明したり原因を特定したりしないでください。
